...
class CertificateTestLetterServiceImplement extends ServiceApi implements CertificateTestLetterService{
     ...
     */
     protected $mainRepository, $testLetterRepository, $templateRepository;

    public function __construct(CertificateTestLetterRepository $mainRepository, TestLetterRepository $testLetterRepository, TemplateCertificateRepository $templateRepository)
    {
    ...
    public function generate_sk($test_letter_id)
    {
        ...
        $template = $this->templateRepository->find(TemplateCertificate::SK_KONVERSI_LAMPIRAN);
        $templatePath = storage_path('app/'.$template->attachment);
        $templateProcessor = new TemplateProcessor($templatePath);

        $templateProcessor->setValue('nomor_surat_sk', Helper::generateNomorSurat($testLetter->queue_number, "SK/SUT"));
        ...
        //        lampiran
        $templateProcessor->setValue('nomor_surat_lampiran', Helper::generateNomorSurat($testLetter->queue_number, "LAMPIRAN/SUT"));
        $templateProcessor->setValue('brand_l', strtoupper($testLetter->certificate->brand));
        $templateProcessor->setValue('type_l', strtoupper($testLetter->certificate->type));
        $templateProcessor->setValue('vehicle_type_l', strtoupper($testLetter->certificate->vehicle_type));
...
//        $templateProcessor->setValue('speedometer_ambang_batas', $testing[4]->a->ambang_batas ?? '');
        ...
        $outputPath = storage_path('app/public/sk-srut-sut/'."Surat_Keterangan_dan_Lampiran_SUT_".$test_letter_id.'.docx');
    ...
    {
        $testLetter = $this->testLetterRepository->find($test_letter_id);
        $template = $this->templateRepository->find(TemplateCertificate::SRUT_KONVERSI);
        $templatePath = storage_path('app/'.$template->attachment);
        $templateProcessor = new TemplateProcessor($templatePath);
    ...
    {
        $testLetter = $this->testLetterRepository->find($test_letter_id);
        $template = $this->templateRepository->find(TemplateCertificate::SUT_KONVERSI);
        $templatePath = storage_path('app/'.$template->attachment);
        $templateProcessor = new TemplateProcessor($templatePath);
    ...
    public function send_draft($test_letter_id)
    {
        ...
        try {
            ...
            if ($result->test_letter->workshop_type == "A" && $result->test_letter->step != "create_certificate_srut") {
                ...
                $result->regarding = "Verifikasi Permohonan SUT";
                $result->status = 'Draft SUT';
            }
...
//                $result->sk_attachment = 'sk-srut-sut/'. $sk['file_name'];
    ...
    public function upsert_form_certificate($data)
    {
        ...
        try {
            ...
            if ($testLetter->workshop_type == "A" && $testLetter->step != "create_certificate_srut") {
                $data['status'] = 'Draft SUT';
                $redirect = redirect()->intended(URL::route('certificate.test.letter.generate', ['id' => Helper::encrypt($data['test_letter_id'])]));
            }

            if ($testLetter->workshop_type == "A" && $testLetter->step == "create_certificate_srut") {
                $data['status'] = 'Draft SRUT';
                $redirect = redirect()->intended(URL::route('certificate.test.letter.certificate.srut', ['id' => Helper::encrypt($data['test_letter_id'])]));
            }

            if ($testLetter->workshop_type == "B") {
                $data['status'] = 'Draft SUT SRUT';
                $redirect = redirect()->intended(URL::route('certificate.test.letter.generate', ['id' => Helper::encrypt($data['test_letter_id'])]));
            }

            $testLetter->status = "Sertifikat Sedang Di Proses";
            $testLetter->message = "Mohon menunggu sertifikat anda sedang di proses oleh tim kami!";
            $testLetter->save();
    ...
    public function table()
    {
        return DataTables::of($this->mainRepository->table())
            ...
            ->addColumn('action', function ($row) {
                $verificationMenu = '';
                if (auth()->user()->isSuperAdmin() && $row->status == 'Draft SRUT' && $row->test_letter->workshop_type == "A" && $row->registration_attachment != null) {
                ...
                if (auth()->user()->isSuperAdmin() && $row->status == 'Draft SUT' && $row->test_letter->workshop_type == "A" && $row->type_test_attachment != null) {
                    $verificationMenu .= '<a class="dropdown-item detail" href="'.route('certificate.test.letter.verification.draft.form', ['id' => Helper::encrypt($row->id)]).'" data-id="'.$row->id.'">
                                  Verifikasi SUT
                                </a>';
                }

                if (auth()->user()->isSuperAdmin() && $row->status == 'Draft SRUT SUT' && $row->test_letter->workshop_type == "B") {
                    $verificationMenu .= '<a class="dropdown-item detail" href="'.route('certificate.test.letter.verification.draft.form', ['id' => Helper::encrypt($row->id)]).'" data-id="'.$row->id.'">
                                  Verifikasi SRUT SUT
                                </a>';
                }
    ...
    public function verify_draft($id)
    {
        ...
        try {
            ...
            if ($result->test_letter->workshop_type == "A" && $result->test_letter->step != "create_certificate_srut") {
                $result->status = 'SUT Terverifikasi';
            }

            if ($result->test_letter->workshop_type == "A" && $result->test_letter->step == "create_certificate_srut") {
                $result->status = 'SRUT Terverifikasi';
            }

            if ($result->test_letter->workshop_type == "B") {
                $result->status = 'SRUT SUT Terverifikasi';
            }
    ...
    public function upload_archive($data)
    {
        ...
        try {
            ...
            if (@$data['type_test_attachment']) {
                ...
                $filePathTypeTest = $fileTypeTest->storeAs('certificate-sut', "Sertifikat_SUT_".$certificate->test_letter->id.'.pdf', 'public');
            ...
            if (@$data['registration_attachment']) {
                ...
                $filePathRegis = $fileRegis->storeAs('certificate-srut', "Sertifikat_SRUT_".$certificate->test_letter->id.'.pdf', 'public');
...
//            }
            ...
            if ($certificate->test_letter->workshop_type == "A" && $certificate->test_letter->step != "create_certificate_srut") {
                ...
                $mail['message'] = '
                                    ...
                                    <p>
                                        Terima kasih atas partisipasi Anda dalam program percepatan kendaraan listrik di Indonesia.
                                        Kami informasikan bahwa dokumen *Surat Uji Tipe (SUT)* dan *Surat Keterangan SUT dan Lampiran SUT* kendaraan listrik Anda telah tersedia dan dapat diunduh melalui tombol di bawah ini:
                                    </p>
    ...
    public function generate_certificate_attachment($test_letter_id)
    {
        $testLetter = $this->testLetterRepository->find($test_letter_id);
        $testing = json_decode($testLetter->certificate->testing);
        $template = $this->templateRepository->find(TemplateCertificate::LAMPIRAN_HASIL_UJI);
        $templatePath = storage_path('app/'.$template->attachment);
        $templateProcessor = new TemplateProcessor($templatePath);
    ...
    public function request_testing($data)
    {
        ...
        try {
            ...
            if ($id == null) {
                $data['testing_status'] = 'Menunggu Hasil Uji';
                $this->mainRepository->create($data);
            } else {
                $this->mainRepository->update($id, $data);
            }
            ...
            return $this->setStatus(true)
                ->setCode(200)
                ->setMessage("Berhasil merequest hasil uji");
        } catch (Exception $e) {
            DB::rollBack();
            return $this->exceptionResponse($e);
        }
    ...
    public function form_testing_submit($data)
    {
        ...
        try {
            ...
            $data['testing'] = json_encode(@$data['pengujian']) ?? null;
            $data['testing_status'] = 'Hasil Uji Sudah Dibuat';
            unset($data['id']);

            $this->mainRepository->update($id, $data);
            $redirect = redirect()->intended(URL::route('testing.index'));
