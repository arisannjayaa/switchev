...
@section('content')
    <div class="container">
        ...
        @php
            $certificate_attachments[] = ['file' => @$test_letter->certificate->type_test_attachment, 'dir' => 'certificate-sut/', 'extension' => pathinfo(@$test_letter->certificate->type_test_attachment, PATHINFO_EXTENSION)];
            $certificate_attachments[] = ['file' => @$test_letter->certificate->registration_attachment, 'dir' => 'certificate-srut/', 'extension' => pathinfo(@$test_letter->certificate->registration_attachment, PATHINFO_EXTENSION)];
            $certificate_attachments[] = ['file' => @$test_letter->certificate->sk_attachment, 'dir' => 'sk-sut-srut/', 'extension' => pathinfo(@$test_letter->certificate->sk_attachment, PATHINFO_EXTENSION)];
            $all_empty = empty(array_filter(array_column($certificate_attachments, 'file')));
        @endphp
        <div class="card">
            <div class="card-header">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    @else
                        @foreach($certificate_attachments as $certificate_attachment)
                            @if($certificate_attachment['file'] != null)
                                @if($certificate_attachment['extension'] == 'pdf')
                                    <a href="{{ route('secure.file', ['path' => \App\Helpers\Helper::encrypt($certificate_attachment['file'])]) }}" class="d-flex align-items-center file-pdf-2 mb-3">
                                        <img width="24" src="{{ asset('assets/dist/img/pdf_illustration.png') }}">
                                        {{ Str::limit(substr($certificate_attachment['file'], strlen($certificate_attachment['dir'])), 60) }}
                                    </a>
                                @endif
                            @endif
                        @endforeach
