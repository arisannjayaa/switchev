...
document.addEventListener('DOMContentLoaded', function() {
    const tableUrl = $('#table-mechanical-url').val();
    const asset = $("#asset-url").val();

    $("#table-mechanical").DataTable({
        order: [[0, 'desc']],
        ordering: true,
        serverSide: true,
        processing: true,
        ...
        columns: [
            ...
            { data: 'task', name: 'task', className: 'text-nowrap', orderable: false, searchable: true},
    ...
    $("#btn-save-mechanical").click(function (e) {
        e.preventDefault();

        let formData = new FormData();
        let btn = "#btn-save-mechanical";
        let table = "#table-mechanical";
        let modal = "#modal-mechanical";

        let id = $("#mechanical_id").val();
        let name = $("#name").val();
        let task = $("#task").val();
        ...
        if (id !== "") {
            var url = $("#update-mechanical-url").val();
        } else {
            var url = $("#create-mechanical-url").val();
        }
    ...
    $("#table-mechanical").on("click", ".edit", function () {
        let id = $(this).data("id");
        let url = $("#edit-mechanical-url").val();
        ...
        fetch(url, {
            ...
            .then(res => {
                if(res.code == 200) {
                    $(".modal-title").empty().append("Edit Tenaga Ahli");
                    $("#mechanical_id").val(res.data.id);
    ...
    $("#table-mechanical").on("click", ".delete", function () {
        let id = $(this).data("id");
        let url = $("#delete-mechanical-url").val();
        let table = "#table-mechanical";
        let formData = new FormData();
        formData.append("id", id);
        loadingScreen();
